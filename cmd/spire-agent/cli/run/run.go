...
func NewAgentConfig(c *Config, logOptions []log.Option, allowUnknownConfig bool) (*agent.Config, error) {
	...
	default:
		return nil, fmt.Errorf("unknown rebootstrap mode specified: %s(%d)", base64.StdEncoding.EncodeToString([]byte(c.Agent.RebootstrapMode)), len(c.Agent.RebootstrapMode))
	}
