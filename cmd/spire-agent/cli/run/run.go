...
func (c *agentConfig) brokerBindAddrs() ([]net.Addr, error) {
	sp, ba := c.Broker.SocketPath, c.Broker.BindAddress
	if sp == "" && ba == "" {
		return nil, errors.New("broker requires socket_path, bind_address, or both")
	}
	var addrs []net.Addr
	if sp != "" {
	...
	if ba != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid broker.bind_address %q: %w", ba, err)
		}
...
func NewAgentConfig(c *Config, logOptions []log.Option, allowUnknownConfig bool) (*agent.Config, error) {
	...
	if c.Agent.Broker != nil {
		...
		seen := make(map[string]struct{}, len(c.Agent.Broker.Brokers))
		for i, b := range c.Agent.Broker.Brokers {
			if b.ID == "" {
				return nil, fmt.Errorf("broker.brokers[%d].id: must be specified", i)
			}
			if _, dup := seen[b.ID]; dup {
				return nil, fmt.Errorf("broker.brokers[%s].id: duplicate broker id", b.ID)
			}
			seen[b.ID] = struct{}{}
		}
		...
		brokers := make([]broker.Broker, 0, len(c.Agent.Broker.Brokers))
		for _, b := range c.Agent.Broker.Brokers {
			if _, err := spiffeid.FromString(b.ID); err != nil {
				return nil, fmt.Errorf("broker.brokers[%s].id: %w", b.ID, err)
			}
			if len(b.AllowedReferenceTypes) == 0 {
				return nil, fmt.Errorf("broker.brokers[%s].allowed_reference_types: must list at least one reference type URL", b.ID)
			}
...
func checkForUnknownConfig(c *Config, l logrus.FieldLogger) (err error) {
	...
	if a := c.Agent; a != nil && a.Broker != nil {
		if len(a.Broker.UnusedKeyPositions) != 0 {
			detectedUnknown("broker", a.Broker.UnusedKeyPositions)
		}
		for i, b := range a.Broker.Brokers {
			if len(b.UnusedKeyPositions) != 0 {
				detectedUnknown(fmt.Sprintf("broker.brokers[%d]", i), b.UnusedKeyPositions)
			}
		}
	}
