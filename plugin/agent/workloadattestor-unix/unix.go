...
func (UnixPlugin) Attest(req *workloadattestor.AttestRequest) (*workloadattestor.AttestResponse, error) {
	...
	if len(uids) == 1 {
		resp.Selectors = append(resp.Selectors, &common.Selector{Type: "unix", Value: fmt.Sprintf("uid:%v", uids[0])})
	} else if len(uids) > 1 {
		// We got at least real and effective UIDs
		// use the effective UID
		resp.Selectors = append(resp.Selectors, &common.Selector{Type: "unix", Value: fmt.Sprintf("uid:%v", uids[1])})
	} else {
	...
	if len(gids) == 1 {
		resp.Selectors = append(resp.Selectors, &common.Selector{Type: "unix", Value: fmt.Sprintf("gid:%v", gids[0])})
	} else if len(gids) > 1 {
		// We got at least real and effective GIDs
		// use the effective GID
		resp.Selectors = append(resp.Selectors, &common.Selector{Type: "unix", Value: fmt.Sprintf("gid:%v", gids[1])})
	} else {
