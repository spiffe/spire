...
func (s *DataStore) TaintX509CA(ctx context.Context, trustDomainID string, authorityID string) error {
	if err := s.getNextError(); err != nil {
		return err
	}
	return s.ds.TaintX509CA(ctx, trustDomainID, authorityID)
}

func (s *DataStore) RevokeX509CA(ctx context.Context, trustDomainID string, authorityID string) error {
	if err := s.getNextError(); err != nil {
		return err
	}
	return s.ds.RevokeX509CA(ctx, trustDomainID, authorityID)
}
