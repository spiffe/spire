...
| `exclude_completed_pods`                | If true, pods in a terminal `status.phase` of `Failed` (e.g. evicted pods or pods left in `ContainerStatusUnknown` after a node problem) or `Succeeded` (e.g. completed jobs) are dropped as the kubelet response is parsed, so they never enter the pod-list cache. Reduces memory usage on nodes that accumulate many terminated pods. Disabled by default; see the note below before enabling. |
...
**Note** Enabling `exclude_completed_pods` trades away the ability to resolve terminal pods in exchange for a smaller memory footprint. Before enabling it, be aware that:

- A pod is dropped as soon as it reaches a terminal phase, even if a process inside it is still running, for example an evicted pod within its termination grace period or a pod left in `ContainerStatusUnknown` after a node problem. Such a workload can no longer be attested, and the `disable_container_selectors` pod-selector fallback cannot fire for it either, because the pod is no longer in the cache.
- Terminal pods can no longer be resolved as Broker API pod references under `pod_reference_scope = "agent_node"` (the default), which has no API server fallback and fails with a "not found on agent node" error. Use `pod_reference_scope = "cluster"` if references to terminal pods must keep resolving.
