...
| `exclude_completed_pods`                | If true, pods in a terminal `status.phase` of `Failed` (e.g. evicted pods or pods left in `ContainerStatusUnknown` after a node problem) or `Succeeded` (e.g. completed jobs) are dropped from the kubelet pod-list cache instead of being tracked. Reduces memory usage on nodes with many terminated pods. Enabled by default. Set to `false` to track terminal-phase pods. |
