...
| `exclude_completed_pods`                | If true, pods in a terminal `status.phase` of `Failed` (e.g. evicted pods or pods left in `ContainerStatusUnknown` after a node problem) or `Succeeded` (e.g. completed jobs) whose containers have all been reaped are dropped as the kubelet response is parsed, so they never enter the pod-list cache. Reduces memory usage on nodes that accumulate many terminated pods. Disabled by default; see the note below before enabling. |
...
* A pod is only dropped once it has reached a terminal phase *and* none of its container statuses still carry a container ID, so a terminal pod whose containers are still running remains attestable. Once a pod is dropped, the `disable_container_selectors` pod-selector fallback can no longer fire for it either, because the pod is no longer in the cache.
