...
| `use_xds`                     | Resolve the SPIRE server via xDS instead of DNS. `server_address` becomes the xDS listener name and `server_port` is ignored; the management server endpoint and node locality come from the gRPC xDS bootstrap (`GRPC_XDS_BOOTSTRAP`). | false                   |
