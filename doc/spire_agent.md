...
| `min_federated_bundle_sync_interval` | Lower bound on how often a federated bundle is refreshed from the SPIRE server. By default the agent refreshes each federated bundle at a quarter of that bundle's refresh hint, as the server does when polling federated bundle endpoints. Raising this will mitigate server and database load in deployments with many federations, but refreshes less often than the federated trust domain asks for. | refresh hint of each bundle |
