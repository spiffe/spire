...
| `log_file`                        | File to write logs to. An external tool can move it aside on any platform, but only POSIX can then tell SPIRE Agent to start a new one, on receipt of `SIGUSR2`. Use `log_file_rotation` on Windows (see below)                                   |                                  |
