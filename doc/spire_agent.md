...
| `min_federated_bundle_sync_interval` | Lower bound on how often a federated bundle is refreshed from the SPIRE server. Raising this will mitigate server and database load for deployments with many federations, but refreshes less often than the federated trust domain asks for. | refresh hint of each bundle |
