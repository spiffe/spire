...
| `insecure_bootstrap` | Enables insecure bootstrapping to the server                   |                      |
...
Either `trust_bundle_path` or `insecure_bootstrap` must be set to configure the agent to bootstrap trust with the server. `insecure_bootstrap` is useful for testing purposes but should be avoided in production since it cannot prevent a man-in-the-middle server. Since attesation often involves passing sensitive materials, it is paramount that the agent trust the server.
