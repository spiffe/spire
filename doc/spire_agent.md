...
| `log_file`                        | File to write logs to. On POSIX platforms, SPIRE Agent reopens this file on receipt of `SIGUSR2`, so an external tool like logrotate can move it aside. That is not possible on Windows; use `log_file_rotation` there (see below)                |                                  |
