...
|:------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `min_tls_version` | Minimum TLS version for **terminating** agent TLS listeners (Prometheus HTTPS, SPIFFE Broker API). Example: `VersionTLS13`.                                                                                 |         |
...
| `require_pq_kem`              | Require use of a post-quantum-safe key exchange method for TLS handshakes on the outbound server gRPC client and on **terminating** agent listeners (Prometheus HTTPS, SPIFFE Broker API). Does not apply TLS profile settings to the outbound client. | false                   |
