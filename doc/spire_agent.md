...
3. If the `trust_bundle_spiffe_workload_api` option is used, the agent will fetch a fresh copy of the trust bundle for its trust domain from the SPIFFE Workload API at the specified endpoint (e.g. `unix:///run/spire/agent.sock`) every time bootstrapping or rebootstrapping is needed. This is useful when nesting SPIRE Agents, such as with the `x509pop` NodeAttestor pointed at an upstream agent's Workload API, without needing a separate adapter daemon.
