...
| `-entryExpiry`   | An expiry, from epoch in seconds, for the resulting registration entry to be pruned. Pruning may be done manually through the API, but also occurs automatically every 5 minutes. If Expiry is not set, the entry is always valid and will never be pruned. | |
