...
| `bundle_cache_ttl`            | How long federated bundle reads are cached before being refreshed from the database. Enabling this will mitigate high database load in deployments with many federations, but will also slow propagation of federated bundle updates to agents. The server warns when this exceeds what a federated bundle's refresh hint leaves after its refresh interval. | disabled                           |
