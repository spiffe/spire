...
// buildAllowedReferenceTypeMap pre-computes the per-caller policy used by
// the api service to gate WorkloadReference type usage. Brokers whose ID
// fails to parse are skipped; the mTLS authorizer will already have rejected
// them at the listener level since the same parse runs in brokerIDsAsSPIFFEIDs.
func buildAllowedReferenceTypeMap(brokers []Broker) map[spiffeid.ID]brokerapi.ReferenceTypePolicy {
	if len(brokers) == 0 {
		return nil
	}
	out := make(map[spiffeid.ID]brokerapi.ReferenceTypePolicy)
	for _, b := range brokers {
		...
		policy := brokerapi.ReferenceTypePolicy{
			Types: make(map[string]brokerapi.ReferenceTypeAccess, len(b.AllowedReferenceTypes)),
		}
		for _, allowed := range b.AllowedReferenceTypes {
			access := brokerapi.ReferenceTypeAccess{AllowOverTCP: allowed.AllowOverTCP}
			if allowed.TypeURL == "*" {
