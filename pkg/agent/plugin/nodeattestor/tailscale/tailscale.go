...
func builtin(p *Plugin) catalog.BuiltIn {
	return catalog.MakeBuiltIn(common_tailscale.PluginName,
...
func buildConfig(coreConfig catalog.CoreConfig, hclText string, status *pluginconf.Status) *Config {
	...
	if newConfig.CertPath == "" {
		status.ReportError("cert_path is required")
	}

	if newConfig.KeyPath == "" {
		status.ReportError("key_path is required")
	}
...
func (p *Plugin) AidAttestation(stream nodeattestorv1.NodeAttestor_AidAttestationServer) (err error) {
	...
	// send the attestation data (cert chain) to the server
...
func loadConfigData(config *Config) (*configData, error) {
	...
	attestationPayload, err := json.Marshal(common_tailscale.AttestationData{
