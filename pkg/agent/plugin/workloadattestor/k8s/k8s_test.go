...
func (s *Suite) TestAttestReferenceWithPodUID_FoundInKubelet() {
	...
	anyRef, err := anypb.New(&reference.KubernetesObjectReference{Resource: &reference.KubernetesResource{Plural: "pods", Group: "core"}, Uid: testPodUID})
	s.Require().NoError(err)
...
`, s.kubeletPort())
	...
	anyRef, err := anypb.New(&reference.KubernetesObjectReference{Resource: &reference.KubernetesResource{Plural: "pods", Group: "core"}, Uid: "nonexistent-uid"})
	s.Require().NoError(err)
