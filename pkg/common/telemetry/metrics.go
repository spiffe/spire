...
func NewMetrics(c *MetricsConfig) (*MetricsImpl, error) {
	...
	for _, f := range sinkRunnerFactories {
		...
		conf.AllowedLabels = c.FileConfig.Options.AllowedLabels
		conf.BlockedLabels = c.FileConfig.Options.BlockedLabels
		conf.AllowedPrefixes = c.FileConfig.Options.AllowedPrefixes
		conf.BlockedPrefixes = c.FileConfig.Options.BlockedPrefixes
