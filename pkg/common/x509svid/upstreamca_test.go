...
func (s *UpstreamCASuite) TestSignCSRWithWrongTrustDomain() {
	...
	cert, err := s.upstreamCA.SignCSR(context.Background(), csr, 0)
	s.requireErrorContains(err, `CSR SPIFFE ID "spiffe://domain.test" is not the trust domain ID for trust domain "example.org"`)
	s.Require().Nil(cert)
}
...
func (s *UpstreamCASuite) TestSignCSRWithWorkloadID() {
	...
	cert, err := s.upstreamCA.SignCSR(context.Background(), csr, 0)
	s.requireErrorContains(err, `CSR SPIFFE ID "spiffe://example.org/foo" is not the trust domain ID for trust domain "example.org"`)
	s.Require().Nil(cert)
}
