...
const (
	eksNodeDefaultListTTL = "1m"  // pull account list after 1 minutes
	eksNodeMinListTTL     = "10s" // Minimum TTL configuration to pull the org account list
	eksRetries            = 5
)

var (
	eksNodeDefaultListDuration, _ = time.ParseDuration(eksNodeDefaultListTTL)
	eksNodeMinTTL, _              = time.ParseDuration(eksNodeMinListTTL)
)
...
func (o *eksValidator) configure(config *eksValidationConfig) error {
	...
	if config.NodesListTTL == "" {
		config.NodesListTTL = eksNodeDefaultListTTL
	}

	t, err := time.ParseDuration(config.NodesListTTL)
	if err != nil {
		return status.Errorf(codes.InvalidArgument, "issue while parsing ttl for EKS, while configuring EKS validation: %v", err)
	}

	if t < eksNodeMinTTL {
		return status.Errorf(codes.InvalidArgument, "ttl for EKS is less than minimum allowed value, while configuring EKS validation")
	}

	o.eksAccountListCacheTTL = t
...
func (o *eksValidator) reloadNodeList(ctx context.Context, eksClient EKSClient, asClient autoscaling.DescribeAutoScalingGroupsAPIClient, catchBurst bool) (map[string]any, error) {
	...
	listNodegroupsOp, err := eksClient.ListNodegroups(ctx, &eks.ListNodegroupsInput{
		ClusterName: &o.eksConfig.EKSClusterName,
	})
	if err != nil {
	...
	for {
		for _, ng := range listNodegroupsOp.Nodegroups {
			instances, err := o.fetchNodesInNodeGroup(ctx, eksClient, asClient, ng)
			if err != nil {
				return nil, err
			}
...
// reloadNodeList gets the list of nodes belonging to the EKS cluster and catch them
func (o *eksValidator) fetchNodesInNodeGroup(ctx context.Context, eksClient EKSClient, asClient autoscaling.DescribeAutoScalingGroupsAPIClient, nodeGroup string) ([]string, error) {
	// Get the list of node groups belonging to the EKS cluster
