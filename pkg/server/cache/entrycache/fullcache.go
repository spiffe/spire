...
type Cache interface {
	FindAuthorizedEntries(agentID spiffeid.ID, entries map[string]struct{}) map[string]*types.Entry
...
func (c *FullEntryCache) FindAuthorizedEntries(agentID spiffeid.ID, entries map[string]struct{}) map[string]*types.Entry {
	seen := allocSeenSet()
	defer freeSeenSet(seen)

	return c.findAuthorizedEntries(spiffeIDFromID(agentID), entries, seen)
}
...
func (c *FullEntryCache) findAuthorizedEntries(id spiffeID, entries map[string]struct{}, seen map[spiffeID]struct{}) map[string]*types.Entry {
	...
	for _, descendant := range c.crawl(id, seen) {
		...
		maps.Copy(foundEntries, c.findAuthorizedEntries(spiffeIDFromProto(descendant.SpiffeId), entries, seen))
	}

	for _, alias := range c.aliases[id] {
		maps.Copy(foundEntries, c.findAuthorizedEntries(alias.id, entries, seen))
	}
	return foundEntries
}
